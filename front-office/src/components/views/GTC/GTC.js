...
const GTC = () => {
  ...
  const [legalMentions, setLegalMentions] = useState(dataLegalMentions)

  useEffect (() => {
    axios({
      method: 'get',
      url: routeName + '/nomDeLaRoute'
    })
      .then((response) => {
        setLegalMentions(response);
      })
      .catch((error) => {
  ...
  return (
    <GTCStyled>
      ...
      <p className="txt-content">{dataLegalMentions}</p>
